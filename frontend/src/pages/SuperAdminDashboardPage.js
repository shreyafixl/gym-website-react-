...
import VendorsManagementPage from "../components/VendorsManagementPage";
...
import ThirdPartyAppsPage from "../components/ThirdPartyAppsPage";
import { SABillingIntegrated, SARevenueIntegrated, SAMaintenanceIntegrated } from "../components/SuperAdminFinanceComponents";
...
// ─── OVERVIEW ─────────────────────────────────────────────────────────────────
function SAOverview({ globalStats = [], branches = [], financialReports = { monthly: [] }, notifications = [] }) {
  // Ensure all data is properly typed
  const statsArray = Array.isArray(globalStats) ? globalStats : [];
  const branchesArray = Array.isArray(branches) ? branches : [];
  const notificationsArray = Array.isArray(notifications) ? notifications : [];
  
  const reports = financialReports || {};
  const rev = Array.isArray(reports.monthly) ? reports.monthly : [];
  const maxRev = rev.length > 0 ? Math.max(...rev.map(r => r?.revenue || 0)) : 1;
    ...
    <div className="sa-section">
      ...
      <div className="sa-kpi-grid">
        {statsArray.map((k, i) => (
          <div className="sa-kpi-card" key={i}>
            <div className="sa-kpi-icon" style={{ background: (k?.color || "#3b82f6") + "22", fontSize: "1.4rem" }}>{k?.icon || "📊"}</div>
            <div>
              <strong>{k?.value || 0}</strong>
              <span>{k?.label || "Metric"}</span>
      ...
      <div className="sa-two-col">
          ...
          <div className="sa-bar-chart">
            {rev.map((v, i) => (
              <div className="sa-bar-col" key={i}>
                <span className="sa-bar-val">${((v?.revenue || 0) / 1000).toFixed(0)}k</span>
                <div className="sa-bar" style={{ height: `${((v?.revenue || 0) / maxRev) * 100}%` }} />
                <span className="sa-bar-label">{v?.month || `M${i + 1}`}</span>
              </div>
            ))}
          </div>
        </div>
        <div className="sa-card">
          ...
          {branchesArray.map(b => (
            <div className="sa-branch-row" key={b?.id || Math.random()}>
              <div><strong>{b?.name || "Unknown"}</strong><span>{b?.city || "N/A"}</span></div>
              <div className="sa-branch-stats">
                <span><FaUsers style={{ marginRight: 4 }} />{b?.members || 0}</span>
                <span>{b?.revenue || "₹0"}</span>
                <span style={{ color: "#22c55e", fontWeight: 700 }}>{b?.growth || "+0%"}</span>
                <SABadge s={b?.status || "inactive"} />
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="sa-card">
        ...
        {notificationsArray.filter(n => !n?.read).map(n => (
          <div key={n?.id || Math.random()} className="sa-alert-row">
            <div className={`sa-alert-dot ${(n?.type === "payment" || n?.type === "alert") ? "sa-red" : "sa-yellow"}`} />
            <div><strong>{n?.title || "Alert"}</strong><span>{n?.message || ""}</span></div>
            <small>{n?.time || "Now"}</small>
          </div>
        ))}
      </div>
    </div>
...
// ─── USER MANAGEMENT ──────────────────────────────────────────────────────────
function SAUsers({ openForm, allUsers = [], branches = [] }) {
...
// ─── BRANCHES ─────────────────────────────────────────────────────────────────
function SABranches({ openForm, branches = [] }) {
  const { toast, show } = useToast();
  ...
  return (
    <div className="sa-section">
      ...
      <div className="sa-branches-grid">
        {branchesArray.map(b => (
          <div className="sa-branch-card sa-card" key={b?.id || Math.random()}>
            <div className="sa-branch-card-head"><h4>{b?.name || "Unknown"}</h4><SABadge s={b?.status || "inactive"} /></div>
            <p className="sa-branch-city"><FaMapMarkerAlt style={{ marginRight: 4 }} />{b?.city || "N/A"}</p>
            <div className="sa-branch-kpis">
              <div><strong>{b?.members || 0}</strong><span>Members</span></div>
              <div><strong>{b?.revenue || "₹0"}</strong><span>Revenue</span></div>
              <div><strong>{b?.trainers || 0}</strong><span>Trainers</span></div>
            </div>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
              ...
              <button className="btn btn-outline sa-btn-sm" onClick={() => show(`Editing ${b?.name || "Branch"}`)}><FaEdit style={{ marginRight: 4 }} />Edit</button>
            </div>
          </div>
        ))}
      </div>
...
// ─── FINANCE: BILLING ─────────────────────────────────────────────────────────
function SABilling({ subscriptions = [] }) {
  ...
  const BillingService = require("../services/superadmin/BillingService").default;
...
// ─── FINANCE: REVENUE ─────────────────────────────────────────────────────────
function SARevenue({ financialReports = { monthly: [] } }) {
  const [period, setPeriod] = useState("monthly");
  
  // Ensure financialReports is an object with monthly array
  const reports = financialReports || {};
...
function SATransactions({ transactions = [] }) {
  const [filter, setFilter] = useState("all");
  const [search, setSearch] = useState("");
  const [page, setPage] = useState(1);
  const PER = 5;
  ...
  const filtered = txnArray.filter(t => {
    try {
  ...
  const paged = filtered.slice((page - 1) * PER, page * PER);
  const total = txnArray.filter(t => t?.status === "success").reduce((a, b) => a + (b?.amount || 0), 0);
  return (
    <div className="sa-section">
      <div className="sa-section-head">
      ...
      <div className="sa-kpi-grid" style={{ gridTemplateColumns: "repeat(4,1fr)" }}>
        <KpiCard icon={<FaCheckCircle />} label="Successful" value={txnArray.filter(t => t?.status === "success").length} color="#22c55e" />
        <KpiCard icon={<FaTimesCircle />} label="Failed" value={txnArray.filter(t => t?.status === "failed").length} color="#ef4444" />
        <KpiCard icon={<FaSync />} label="Refunded" value={txnArray.filter(t => t?.status === "refunded").length} color="#f97316" />
            ...
            <tbody>
              {paged.map(t => (
                <tr key={t?.id || Math.random()}>
                  <td><code style={{ fontSize: ".75rem", color: "var(--accent)" }}>{t?.id || "N/A"}</code></td>
                  <td><strong>{t?.user || "Unknown"}</strong></td>
                  <td>{t?.plan || "N/A"}</td>
                  <td><strong style={{ color: t?.status === "failed" ? "#ef4444" : "var(--text-primary)" }}>${t?.amount || 0}</strong></td>
                  <td style={{ color: "var(--text-secondary)", fontSize: ".8rem" }}>{t?.method || "N/A"}</td>
                  <td style={{ color: "var(--text-secondary)", fontSize: ".78rem" }}>{t?.date || "N/A"}</td>
...
function SAPlans({ openForm, membershipPlans = [] }) {
  ...
  return (
    <div className="sa-section">
      ...
      <div className="sa-plans-grid">
        {plans.length === 0 ? (
          <div className="sa-card" style={{ textAlign: "center", padding: "40px 20px", color: "var(--text-secondary)", gridColumn: "1 / -1" }}>
            <div style={{ fontSize: "2rem", marginBottom: 12 }}>📋</div>
            <p>No membership plans yet</p>
            <p style={{ fontSize: ".85rem", marginTop: 8 }}>Create your first membership plan to get started</p>
          </div>
        ) : (
          plans.map(p => (
...
// ─── ANALYTICS: REPORTS ───────────────────────────────────────────────────────
function SAReports({ financialReports = { monthly: [] }, memberGrowth = [] }) {
  const { toast, show } = useToast();
  const summaries = [
...
function SAMemberAnalytics({ memberGrowth = [] }) {
  // Ensure memberGrowth is an array
  const growthArray = Array.isArray(memberGrowth) ? memberGrowth : [];
  const maxActive = growthArray.length > 0 ? Math.max(...growthArray.map(d => d?.active || 0)) : 1;
  return (
    <div className="sa-section">
      <div className="sa-section-head">
      ...
      <div className="sa-two-col">
        <div className="sa-card">
          ...
          <div className="sa-bar-chart" style={{ height: 140 }}>
            {growthArray.map((d, i) => (
              <div className="sa-bar-col" key={i}>
                <div style={{ display: "flex", gap: 1, alignItems: "flex-end", height: "100%" }}>
                  <div className="sa-bar" style={{ height: `${((d?.active || 0) / maxActive) * 100}%`, background: "#22c55e", flex: 2 }} />
                  <div className="sa-bar" style={{ height: `${((d?.inactive || 0) / maxActive) * 100}%`, background: "#ef4444", flex: 1 }} />
                </div>
                <span className="sa-bar-label">{d?.month || `M${i + 1}`}</span>
              </div>
            ))}
          </div>
          <div style={{ display: "flex", gap: 16, marginTop: 8, fontSize: ".75rem" }}>
        ...
        <div className="sa-card">
          ...
          <div className="sa-bar-chart" style={{ height: 140 }}>
            {growthArray.map((d, i) => {
              const maxNew = growthArray.length > 0 ? Math.max(...growthArray.map(x => x?.new || 0)) : 1;
              return (
                <div className="sa-bar-col" key={i}>
                  <span className="sa-bar-val">{d?.new || 0}</span>
                  <div className="sa-bar" style={{ height: `${((d?.new || 0) / maxNew) * 100}%`, background: "#8b5cf6" }} />
                  <span className="sa-bar-label">{d?.month || `M${i + 1}`}</span>
                </div>
              );
            })}
...
function SAFinancialAnalytics({ financialReports = { monthly: [] }, membershipPlans = [] }) {
  // Ensure data is properly typed
  const reports = financialReports || {};
  const data = Array.isArray(reports.monthly) ? reports.monthly : [];
  const maxProfit = data.length > 0 ? Math.max(...data.map(d => d?.profit || 0)) : 1;
  
  // Ensure membershipPlans is an array
  const plansArray = Array.isArray(membershipPlans) ? membershipPlans : [];
  ...
  return (
    <div className="sa-section">
      ...
      <div className="sa-two-col">
        <div className="sa-card">
          ...
          <div className="sa-bar-chart" style={{ height: 140 }}>
            {data.map((d, i) => (
              <div className="sa-bar-col" key={i}>
                <span className="sa-bar-val">${(((d?.profit || 0)) / 1000).toFixed(0)}k</span>
                <div className="sa-bar" style={{ height: `${((d?.profit || 0) / maxProfit) * 100}%`, background: "#22c55e" }} />
                <span className="sa-bar-label">{d?.month || `M${i + 1}`}</span>
              </div>
            ))}
          </div>
        ...
        <div className="sa-card">
          ...
          <div className="sa-bar-chart" style={{ height: 140 }}>
            {forecast.map((f, i) => (
              <div className="sa-bar-col" key={i}>
                <span className="sa-bar-val">${(((f?.forecast || 0)) / 1000).toFixed(0)}k</span>
                <div className="sa-bar" style={{ height: `${((f?.forecast || 0) / 65000) * 100}%`, background: f?.actual ? "#ef4444" : "#8b5cf6", opacity: f?.actual ? 1 : 0.65 }} />
                <span className="sa-bar-label">{f?.month || `M${i + 1}`}</span>
              </div>
            ))}
          </div>
      ...
      <div className="sa-card">
        ...
        {plansArray.length === 0 ? (
          <EmptyState title="No membership plans" desc="Plans will appear here" />
        ) : (
          plansArray.map((p, i) => {
            const rev = (p?.price || 0) * (p?.members || 0);
            const maxRev = plansArray.length > 0 ? Math.max(...plansArray.map(x => (x?.price || 0) * (x?.members || 0))) : 1;
            const colors = ["#ef4444","#3b82f6","#22c55e","#8b5cf6","#f97316","#06b6d4"];
            return (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}>
                <span style={{ width: 90, fontSize: ".8rem" }}>{p?.name || "Plan"}</span>
                <div style={{ flex: 1, background: "var(--bg-primary)", borderRadius: 4, height: 10 }}>
...
function SANotificationsCenter({ notifications = [] }) {
...
function SACampaigns({ openForm, campaigns = [] }) {
  const [showModal, setShowModal] = useState(false);
  ...
  return (
    <div className="sa-section">
      ...
      <div className="sa-card">
        {campaigns.length === 0 ? (
          <EmptyState title="No campaigns created" desc="Create your first campaign to get started" />
        ) : (
          <table className="sa-table">
...
function SAEquipment({ openForm, equipment = [] }) {
  const [filter, setFilter] = useState("all");
  const [search, setSearch] = useState("");
  const { toast, show } = useToast();
...
// ─── OPERATIONS: VENDORS ──────────────────────────────────────────────────────
// Moved to VendorsManagementPage.js

// ─── OPERATIONS: MAINTENANCE ──────────────────────────────────────────────────
// Moved to MaintenanceManagementPage.js

// ─── INTEGRATIONS: API SETTINGS ───────────────────────────────────────────────
// Moved to ApiSettingsPage.js

// ─── INTEGRATIONS: THIRD-PARTY APPS ──────────────────────────────────────────
// Moved to ThirdPartyAppsPage.js
...
function SAAIInsights({ aiInsights = [] }) {
  const { toast, show } = useToast();
      ...
      <div className="sa-card" style={{ background: "linear-gradient(135deg, rgba(239,68,68,.08), rgba(139,92,246,.08))", border: "1px solid rgba(139,92,246,.2)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          ...
          <div>
            ...
            <p style={{ margin: 0, fontSize: ".8rem", color: "var(--text-secondary)" }}>Last analyzed: Today at 6:00 AM · {aiInsights.length} insights generated</p>
          </div>
          <span className="sa-badge sa-green" style={{ marginLeft: "auto" }}>Active</span>
        </div>
      </div>
      {aiInsights.length === 0 ? (
        <EmptyState title="No AI insights available" desc="AI analysis will generate insights as data accumulates" />
      ) : (
...
function SAFeatureFlags({ openForm, featureFlags = [] }) {
  ...
  return (
    <div className="sa-section">
      ...
      <div className="sa-card">
        {flags.length === 0 ? (
          <EmptyState title="No feature flags configured" desc="Add feature flags to control feature rollout" />
        ) : (
          <table className="sa-table">
...
function SALiveMonitoring({ liveMonitoring = [] }) {
  const [tick, setTick] = useState(0);
  const { toast, show } = useToast();
  
  // Handle both array and object formats
  const isArray = Array.isArray(liveMonitoring);
  const data = isArray ? {} : liveMonitoring;
  const activeNow = (data.activeUsers || 0) + Math.floor(Math.sin(tick) * 5);
  ...
  return (
    <div className="sa-section">
      ...
      {isArray || !data.activeUsers ? (
        <EmptyState title="No live monitoring data" desc="Live monitoring will display when users are active" />
      ) : (
        <>
          <div className="sa-kpi-grid">
            <KpiCard icon={<FaUsers />} label="Active Users Now" value={activeNow} color="#22c55e" change="Real-time" />
            <KpiCard icon={<FaCalendarAlt />} label="Check-ins Today" value={data.checkInsToday || 0} color="#3b82f6" change="Across all branches" />
            <KpiCard icon={<FaHeartbeat />} label="Peak Hour" value={data.peakHour || "N/A"} color="#f97316" />
            <KpiCard icon={<FaServer />} label="Server Load" value={`${data.currentLoad || 0}%`} color={(data.currentLoad || 0) > 80 ? "#ef4444" : "#22c55e"} change={(data.currentLoad || 0) > 80 ? "High load!" : "Normal"} />
          </div>
          <div className="sa-card">
            <div className="sa-card-head"><h3>Branch Live Status</h3></div>
            {(data.branchLive || []).length === 0 ? (
              <EmptyState title="No branch data" desc="Branch live status will appear when data is available" />
            ) : (
              (data.branchLive || []).map((b, i) => (
...
function SALoginHistory({ loginHistory = [] }) {
  return (
    <div className="sa-section">
      <div className="sa-section-head">
      ...
      <div className="sa-card">
        {loginHistory.length === 0 ? (
          <EmptyState title="No login history found" desc="Login attempts will appear here" />
        ) : (
          <table className="sa-table">
...
function SASystemLogs({ systemLogs = [] }) {
  const [filter, setFilter] = useState("all");
  ...
  return (
    <div className="sa-section">
      ...
      <div className="sa-card">
        {filtered.length === 0 ? (
          <EmptyState title="No system logs found" desc="System logs will appear when events occur" />
        ) : (
          filtered.map(l => (
...
// ─── SETTINGS ─────────────────────────────────────────────────────────────────
function SASettings({ systemSettings = {} }) {
  const { toast, show } = useToast();
  return (
    <div className="sa-section">
      {toast && <Toast msg={toast} onClose={() => {}} />}
...
// ─── SUPPORT: TICKETS ─────────────────────────────────────────────────────────
function SATickets({ openForm, supportTickets = [] }) {
  const [filter, setFilter] = useState("all");
  const [search, setSearch] = useState("");
  const { toast, show } = useToast();
...
// ─── SUPPORT: FEEDBACK ────────────────────────────────────────────────────────
function SAFeedback({ feedbackData = [] }) {
  const avgRating = feedbackData.length > 0 ? (feedbackData.reduce((a, b) => a + b.rating, 0) / feedbackData.length).toFixed(1) : 0;
  return (
    <div className="sa-section">
      ...
      {feedbackData.length === 0 ? (
        <EmptyState title="No feedback received" desc="Member feedback will appear here" />
      ) : (
        <>
          <div className="sa-card">
...
// ─── SECTION ROUTER ───────────────────────────────────────────────────────────
function renderSection(active, openForm, dashboardData) {
  const {
    globalStats,
    branches,
    allUsers,
  ...
  const map = {
    overview:             <SAOverview globalStats={globalStats} branches={branches} financialReports={financialReports} notifications={notifications} />,
    users:                <SAUsers openForm={openForm} allUsers={allUsers} branches={branches} />,
    branches:             <SABranches openForm={openForm} branches={branches} />,
    content:              <SAContent openForm={openForm} />,
    billing:              <SABillingIntegrated subscriptions={subscriptions} />,
    revenue:              <SARevenueIntegrated financialReports={financialReports} />,
    transactions:         <SATransactions transactions={transactions} />,
    plans:                <SAPlans openForm={openForm} membershipPlans={membershipPlans} />,
    reports:              <SAReports financialReports={financialReports} memberGrowth={[]} />,
    "members-analytics":  <SAMemberAnalytics memberGrowth={[]} />,
    "fin-analytics":      <SAFinancialAnalytics financialReports={financialReports} membershipPlans={membershipPlans} />,
    "notifications-center": <SANotificationsCenter notifications={notifications} />,
    campaigns:            <SACampaigns openForm={openForm} campaigns={campaigns} />,
    communication:        <SACommunication />,
    equipment:            <SAEquipment openForm={openForm} equipment={equipment} />,
    vendors:              <VendorsManagementPage />,
    maintenance:          <SAMaintenanceIntegrated />,
    "api-settings":       <ApiSettingsPage />,
    "third-party":        <ThirdPartyAppsPage />,
    "import-data":        <SAImportData />,
    "export-data":        <SAExportData />,
    backups:              <SABackups />,
    "ai-insights":        <SAAIInsights aiInsights={aiInsights} />,
    "feature-flags":      <SAFeatureFlags openForm={openForm} featureFlags={featureFlags} />,
    "live-monitoring":    <SALiveMonitoring liveMonitoring={liveMonitoring} />,
    audit:                <SAAudit auditLog={auditLog} />,
    "login-history":      <SALoginHistory loginHistory={loginHistory} />,
    "system-logs":        <SASystemLogs systemLogs={systemLogs} />,
    settings:             <SASettings systemSettings={systemSettings} />,
    "notif-settings":     <SANotifSettings />,
    "sys-config":         <SASysConfig />,
    tickets:              <SATickets openForm={openForm} supportTickets={supportTickets} />,
    feedback:             <SAFeedback feedbackData={feedbackData} />,
  };
  return map[active] || (
    <div className="sa-section">
...
function TopbarBell({ onNav, notifications = [] }) {
...
export default function SuperAdminDashboardPage() {
  ...
  // Load dynamic dashboard data from hook
  const {
    globalStats,
    branches,
    allUsers,
  ...
  if (!isLoggedIn || user?.role !== 'superadmin') {
    console.log('❌ SuperAdminDashboardPage: Access denied', {
      isLoggedIn,
      userRole: user?.role,
    });
      ...
      <div className="sa-layout">
        ...
        <div className="sa-main">
          <header className="sa-topbar">
            ...
            <span className="sa-topbar-title">Super Admin · Loading...</span>
    ...
    return (
      <div className="sa-layout">
        ...
        <div className="sa-main">
          <header className="sa-topbar">
            <button className="sa-menu-btn" onClick={() => setSidebarOpen(true)}>☰</button>
            <span className="sa-topbar-title">Super Admin · Error</span>
            <div className="sa-topbar-right">
              <DashboardThemeSwitcher themeId={themeId} setThemeId={setThemeId} themes={themes} />
              <div className="sa-avatar sa-avatar-sm">SA</div>
            </div>
          ...
          <main className="sa-content">
            <div className="sa-section">
              <div style={{ textAlign: "center", padding: "40px 20px" }}>
                <div style={{ fontSize: "2rem", marginBottom: "10px", color: "#ef4444" }}>⚠️</div>
  ...
  const handleFormSubmit = async (formData) => {
    try {
      ...
      if (activeForm === "createUser") {
        // Import UserService
        const UserService = require("../services/superadmin/UserService").default;
        
        console.log('📝 [handleFormSubmit] Creating user with data:', formData);
        
        // Call API to create user
        const response = await UserService.create(formData);
        
        console.log('✅ [handleFormSubmit] User creation response:', {
        ...
        if (response?.success && response?.data?.user) {
          console.log('🔄 [handleFormSubmit] Refreshing user list after creation...');
          
          // Refresh user list
          await fetchUsers();
          
          console.log('✅ [handleFormSubmit] User list refreshed');
          
          closeForm();
          setToast(`✅ User "${formData.fullName}" created successfully!`);
          setTimeout(() => setToast(null), 3500);
        } else {
          console.warn('⚠️  [handleFormSubmit] Unexpected response structure:', response);
          setToast(`⚠️  User may have been created but response was unexpected`);
          setTimeout(() => setToast(null), 3500);
        }
      } else if (activeForm === "createBranch") {
        // Import BranchService
        const BranchService = require("../services/superadmin/BranchService").default;
        
        console.log('📝 [handleFormSubmit] Creating branch with data:', formData);
        
        // Call API to create branch
        const response = await BranchService.create(formData);
        
        console.log('✅ [handleFormSubmit] Branch creation response:', {
          success: response?.success,
          hasData: !!response?.data,
          hasBranch: !!response?.data?.branch,
        });
        
        if (response?.success && response?.data?.branch) {
          console.log('🔄 [handleFormSubmit] Refreshing branch list after creation...');
          
          // Refresh branch list
          await fetchBranches();
          
          console.log('✅ [handleFormSubmit] Branch list refreshed');
          
          closeForm();
          setToast(`✅ Branch "${formData.branchName}" created successfully!`);
          setTimeout(() => setToast(null), 3500);
        } else {
          console.warn('⚠️  [handleFormSubmit] Unexpected response structure:', response);
          setToast(`⚠️  Branch may have been created but response was unexpected`);
          setTimeout(() => setToast(null), 3500);
        }
      } else {
        // For other forms, just close and show success
        closeForm();
        setToast(`✅ ${formTitles[activeForm] || "Action"} completed successfully!`);
        setTimeout(() => setToast(null), 3500);
      }
    } catch (error) {
      console.error("❌ [handleFormSubmit] Error submitting form:", {
        error: error.message,
    ...
    <div className="sa-layout">
      ...
      <div className="sa-main">
        <header className="sa-topbar">
          ...
          <div className="sa-topbar-right">
            <DashboardThemeSwitcher themeId={themeId} setThemeId={setThemeId} themes={themes} />
            <TopbarBell onNav={go} notifications={notifications} />
            <div className="sa-avatar sa-avatar-sm">SA</div>
          </div>
        </header>
        <main className="sa-content">
